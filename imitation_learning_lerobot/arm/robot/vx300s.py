...
class VX300S(Robot):
    def __init__(self) -> None:
        ...
        theta2_offset = np.arctan2(0.3, 0.05955)
    ...
    def ikine(self, Tep: SE3) -> np.ndarray:
        ...
        theta1_condition = np.power(wpc[1], 2) + np.power(wpc[0], 2) - np.power(self.d_array[2], 2)
        if theta1_condition < 0:
            return np.array([])
        if MathUtils.near_zero(np.linalg.norm([wpc[1], wpc[0]])):
            thetas[0] = self.q0[0] - self.theta_array[0]
        else:
            thetas[0] = np.arctan2(wpc[1], wpc[0]) - np.arctan2(self.d_array[2],
                                                                self.robot_config.overhead * np.sqrt(theta1_condition))

        k3 = (np.power(self.a_array[1] - np.cos(thetas[0]) * wpc[0] - np.sin(thetas[0]) * wpc[1], 2) + np.power(wpc[2],
                                                                                                                2) - (
        ...
        g = np.cos(thetas[0]) * wpc[0] + np.sin(thetas[0]) * wpc[1] - self.a_array[1]
        ...
        thetas[1] = np.arctan2(g * f - wpc[2] * e, g * e + wpc[2] * f)
    ...
    def set_robot_config(self, q):
        ...
        if wrap(wrap(thetas[0])[0] - np.arctan2(wpc[1], wpc[0]))[0] <= np.pi / 2:
